...
df.merged <- readRDS("data/df_dwd.rds")
```
...
```{r barplot1, echo=TRUE, message=FALSE, warning=FALSE, eval = TRUE}
...
```{r histogram_hoehe, echo=TRUE, message=FALSE, warning=FALSE, eval = TRUE}
...
```{r histogram_niederschlag, echo=TRUE, message=FALSE, warning=FALSE, eval = TRUE}
...
```{r barplot2, echo=TRUE, message=FALSE, warning=FALSE, eval = TRUE}
df.merged %>%
  group_by(Bundesland) %>%
...
```{r interaktiv, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE}
library(maps)
library(widgetframe)
library(rbokeh)
